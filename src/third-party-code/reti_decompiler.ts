...
function toSigned(num: number) {
    return num - Math.pow(2, 21)
}

function decompileLoad(instruction: number) {
...
function decompileStore(instruction: number) {
    let mode = (instruction >>> 28) & 0x3
...
function decompileJump(instruction: number) {
...
function decompileCompute (instruction: number) {
...
function decompile (instruction: number) {
